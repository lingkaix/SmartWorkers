...
render_template() {
  ...
  sed \
    ...
    -e "s/{{WORKSPACE_TEMPLATE_SPEC_VERSION}}/${workspace_template_spec_version}/g" \
    "$template_path"
}
...
generate_gitignore() {
  ...
  printf '# workspace-setup version: %s\n' "$workspace_setup_version"
  printf '# template spec version: %s\n' "$workspace_template_spec_version"
  cat <<'EOF'
#
