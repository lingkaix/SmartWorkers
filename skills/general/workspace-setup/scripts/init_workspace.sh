...
generate_ignore_from_gitignore() {
...
# recognize (e.g. `temp/`, `artifacts/`, local configs). This `.ignore` file re-includes
# everything excluded by `.gitignore`.
#
...
EOF
  ...
  if [[ ! -f "$gitignore_path" ]]; then
    echo "# No .gitignore found at: $gitignore_path"
    return 0
  fi
  ...
  awk '
    /^[[:space:]]*#/ { next }
    /^[[:space:]]*$/ { next }
    /^\!/ { next } # already an "unignore" rule
    { print "!"$0; if ($0 ~ /\/$/) print "!"$0"**" }
  ' "$gitignore_path"
}
...
generate_ignore_from_gitignore "$gitignore_target" | write_if_missing "$ignore_target" ".ignore"
